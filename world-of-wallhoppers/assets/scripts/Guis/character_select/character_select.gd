...
func start_game():
	...
	for index in range(PLAYER_COUNT):
		...
		session_info.characters.append(dials[current_dial_selection[index]].character_def)
...
func player_tester_process():
	if ready_players[0]:
		if dials[current_dial_selection[0]].character_def.scene == null:
			ready_players[0] = false
		else:
			character_tester_player_1.start(dials[current_dial_selection[0]].character_def.scene, 0)
	else:
		character_tester_player_1.stop()
	
	if is_multiplayer and ready_players[1]:
		if dials[current_dial_selection[1]].character_def.scene == null:
			ready_players[1] = false
		else:
			character_tester_player_2.start(dials[current_dial_selection[1]].character_def.scene, 1)
	else:
